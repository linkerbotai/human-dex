...
class DynamixelClient:
    ...
    def write_desired_pgain(self, motor_ids: Sequence[int],
                          pgains: np.ndarray):
        """Writes the given desired positions.

        Args:
            motor_ids: The motor IDs to write to.
            pgains: The joint pgains.
        """
        assert len(motor_ids) == len(pgains)

        # Convert to Dynamixel position space.
        self.sync_write(motor_ids, pgains, ADDR_POSITION_P_GAIN,
                        LEN_POSITION_P_GAIN)
        
    def write_desired_dgain(self, motor_ids: Sequence[int],
                          dgains: np.ndarray):
        """Writes the given desired positions.

        Args:
            motor_ids: The motor IDs to write to.
            dgains: The joint dgains.
        """
        assert len(motor_ids) == len(dgains)

        # Convert to Dynamixel position space.
        self.sync_write(motor_ids, dgains, ADDR_POSITION_D_GAIN,
                        LEN_POSITION_D_GAIN)
