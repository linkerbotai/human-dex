...
class Recorder:
    ...
    def list_removes(self, data, indices_to_remove=[6, 13, 14, 15, 16]):
        # 使用列表推导式，只保留不在 indices_to_remove 中的元素
        return tuple(item for i, item in enumerate(data) if i not in indices_to_remove)
    # state
    def joint_state_cb(self, data):
        ...
        # self.data = data
        if self.joint_state_data == None:
            # self.name = list(self.list_removes(data=data.name))
            self.name = ["joint11", "joint12",
                         "joint13", "joint14", "joint15", "joint16", "joint21", "joint22", "joint23", "joint24", "joint25", "joint26"]
            self.qpos = list(self.list_removes(data=data.position))
            self.qvel = list(self.list_removes(data=data.velocity))
            self.effort = list(self.list_removes(data=data.effort))
            
        else:
            self.name = list(self.joint_state_data.name)
            self.qpos = self.joint_state_data.position
            self.qvel = self.joint_state_data.velocity
            self.effort = self.joint_state_data.effort
